...
class OffPolicyDistillationTrainer(Trainer):
    def __init__(
        ...
        *args: Any,
        ...
        distillation_top_k: int,
        distillation_temperature: float,
        **kwargs: Any,
    ) -> None:
        super().__init__(*args, **kwargs)
        self.teacher_model = teacher_model
        self.distillation_top_k = distillation_top_k
    ...
    def compute_loss(
        ...
        with torch.no_grad():
            teacher_outputs = self.teacher_model(**model_inputs, use_cache=False)

        loss = topk_soft_cross_entropy(
            student_logits=student_outputs.logits,
            teacher_logits=teacher_outputs.logits,
            labels=labels,
            top_k=self.distillation_top_k,
...
def run(settings: Settings) -> None:
    ...
    trainer = OffPolicyDistillationTrainer(
        ...
        distillation_top_k=settings.distillation.top_k,
...
def main() -> None:
    ...
    print(
        ...
        f"student {settings.model.name}, teacher {settings.teacher.name}, "
        f"{settings.dataset.max_examples} fixed examples, "
        f"top-{settings.distillation.top_k} targets."
    )
